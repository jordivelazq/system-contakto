...
class InvestigacionFacturaTemplateView(GroupRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        context = super(InvestigacionFacturaTemplateView, self).get_context_data(**kwargs)
...
class InvestigacionFacturaViewSet(mixins.ListModelMixin, viewsets.GenericViewSet):
    ...
    def get_queryset(self):
        ...
        qs = self.queryset.filter(
                 cliente_solicitud__isnull=False, investigacion_completada=True).order_by("last_modified")
...
class InvestigacionFacturaDireccionFiscalUpdateView(GroupRequiredMixin, UpdateView):
    ...
    def get_context_data(self, **kwargs):
        context = super(InvestigacionFacturaDireccionFiscalUpdateView, self).get_context_data(**kwargs)

        investigacion  = Investigacion.objects.get(id=self.kwargs['investigacion_id'])

        context['investigacion_id'] = self.kwargs['investigacion_id']

        context['form'].fields['direccion_fiscal'].queryset = DireccionFiscal.objects.filter(compania_id=investigacion.compania_id)
